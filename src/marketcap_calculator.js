function formatVolume(volume) {
  ...
  export default function calculateMarketCap(tradingData) {
    ...
    const df = filteredData.map(data => ({
      ...
      marketCap: parseFloat(data.c) * parseFloat(data.v) // Assuming current price * base volume for market cap
    }));
  
    const maxPriceChange = Math.max(...df.map(row => Math.abs(row.priceChangePercentage)));
    ...
    const normalizationFactor = maxPriceChange + maxVolume + maxTradeCount + maxBidAskSpread;
    ...
    const result = df.map(row => ({
      ...
      volumePriceRatio: (row.baseVolume / row.currentPrice).toFixed(8),
      volatilityScore: (
        (Math.abs(row.priceChangePercentage) + row.baseVolume + row.numberOfTrades + row.bidAskSpread) 
        / normalizationFactor
      ).toFixed(7),
      marketCap: formatVolume(row.marketCap)
    }));
  
    return result.sort((a, b) => b.marketCap - a.marketCap).slice(0, 100);
  }